...
Deltas live documentés : CBIBF +7, ORAC +7, SICC +17, MVSC +15, UNLC +3, ORGT +12, SHEC +2, STAC +15, NSBC +3, ECOC +10, TTLS +2, SIBC +3. BIIC, BOAS, BOAM, SCRC, SIVC et SEMC ont été revérifiés sans nouveau delta à leur passe courante.
...
- SHA calculés et vérifiés : **685 / 3 046** (**22,49 %**) ;
- restant non hashé sur l'état V13 : **2 361 PDF** ;
- TRITRAF 8/8 ; CBIBF 15/15 ; ORAC 21/21 ; SICC 36/36 ; MVSC 35/35 ; UNLC 23/23 ; ORGT 35/35 ; SHEC 39/39 ; STAC 53/53 ; NSBC 41/41 ; ECOC 42/42 ; BIIC 2/2 ; BOAS 43/43 ; BOAM 44/44 ; SCRC 45/45 ; TTLS 47/47 ; SIBC 49/49 ; SIVC 53/53 ; SEMC **54/54**.
...
SIVC ajoute **0** groupe exact : 53 fichiers / 53 SHA uniques. SEMC ajoute également **0** groupe exact : 54 fichiers / 54 SHA uniques.

Tous les objets physiques restent conservés dans SOURCE ; registre global `inventory/p1_duplicate_groups.csv`.

### Résultats récents SEMC

- live strict : **54 PDF**, identique au snapshot précédent ; delta **0** ; total V13 reste **3 046 PDF** ;
- 25 dossiers directs contrôlés individuellement : 24 années + `divers` ; 2002 absent, aucun dossier 2024/2025 observé ;
- 54/54 tailles Drive ↔ fichiers validées ; **54 SHA uniques**, aucun doublon exact ; registre `inventory/hashes/SEMC.csv` ;
- les variantes historiques `_2`, `_3`, `_4`, `_5` sont binaires distinctes ;
- `divers_Attestation_CAC_Annuel_SEMC.pdf` : 3 pages, sans couche texte exploitable, revue visuelle ; le contenu prouve une attestation CAC sur le tableau d'activité et de résultat et le rapport d'activité semestriel **au 30 juin 2020**, couvrant **du 1er janvier au 30 juin 2020** ; période résolue **S1 2020** ; attestation datée du **3 décembre 2020** ;
- la règle contenu > nom/dossier est confirmée : le libellé `Annuel` du nom de fichier ne décrit pas correctement la période économique du document.
...
1. traiter **BOABF** (57 PDF dans le manifeste V13), plus petit corpus non hashé suivant ;
...
`48/48 ISSUERS | LIVE_TOTAL=3046 | MASTER_CONSOLIDATED=14/3046 | SHA256_VERIFIED=685/3046 | SHA_COVERAGE=22.49% | EXACT_DUPLICATE_GROUPS=9 | SEMC=54/54_SHA_COMPLETE | NEXT=BOABF_LIVE_RECHECK_AND_SHA`
