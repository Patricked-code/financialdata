...
- [x] 48 / 48 sociétés inventoriées ; état live **V16 = 3 070 PDF**.
- [x] checkpoint `inventory/P1_48_ISSUERS_CHECKPOINT_v16_20260817.md` ; index `inventory/p1_issuer_manifest.csv`.
...
- [ ] continuer la revérification live société par société, prochain corpus : **SLBC** (snapshot manifeste 65), puis PRSC (68).

### Document manifest

- [x] schéma/sharding/maître créés ; maître consolidé **14 / 3 070**.
- [x] registres SHA avec Drive IDs + tailles disponibles pour les corpus hashés, y compris BICC et CIEC.
...
- [x] total SHA calculés : **1 127 / 3 070 = 36,71 %**.
- [x] restant non hashé : **1 943 PDF**.
...
- [ ] prochain : **SLBC** après revérification live stricte.

### Doublons / versions

- [x] treize groupes exacts documentés dans `inventory/p1_duplicate_groups.csv`.
...
`SLBC_LIVE_RECHECK_AND_SHA` puis `PRSC_LIVE_RECHECK_AND_SHA`, ensuite poursuite des corpus réellement non hashés par taille croissante.
