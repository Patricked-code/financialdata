...
- [x] 48 / 48 sociétés inventoriées ; état live **V17 = 3 075 PDF**.
- [x] checkpoint `inventory/P1_48_ISSUERS_CHECKPOINT_v17_20260817.md` ; index `inventory/p1_issuer_manifest.csv`.
...
- [ ] continuer la revérification live société par société, prochain corpus : **PRSC** (snapshot manifeste 68), puis SAFC (73).

### Document manifest

- [x] schéma/sharding/maître créés ; maître consolidé **14 / 3 075**.
- [x] registres SHA avec Drive IDs + tailles disponibles pour les corpus hashés, y compris BICC, CIEC et SLBC.
...
- [x] total SHA calculés : **1 197 / 3 075 = 38,93 %**.
- [x] restant non hashé : **1 878 PDF**.
...
- [x] SLBC registre post-commit validé via blob `6b54e3cfc5f6616e274846ec041b1b758b491251` après normalisation CRLF→LF de la sérialisation locale.
- [ ] prochain : **PRSC** après revérification live stricte.

### Doublons / versions

- [x] treize groupes exacts documentés dans `inventory/p1_duplicate_groups.csv` ; SLBC n'en ajoute aucun.
...
- [x] CIEC EF 2017 plain / `_rev` : binaires distincts, même publication économique sous représentation physique différente ; `VERSION_OF`, pas `EXACT_DUPLICATE`.
- [x] CIEC EF 2017 : ne pas renseigner automatiquement `supersedes_source_file_id`, aucune formule explicite `annule et remplace` observée.
- [x] BICC EF 2019 plain / `_rev` : `VERSION_OF`, pas doublon exact ni supersedes automatique.
- [x] SLBC S1 2020 : deux binaires distincts (`b244c5…` / `c3eefc…`) mais page rendue identique ; `VERSION_OF`, pas `EXACT_DUPLICATE`, pas `SUPERSEDES` sans preuve.
...
`PRSC_LIVE_RECHECK_AND_SHA` puis `SAFC_LIVE_RECHECK_AND_SHA`, ensuite poursuite des corpus réellement non hashés par taille croissante.
